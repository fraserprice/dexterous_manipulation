...
class SimpleRobot2D(Env):
    ...
    def __init__(self, action_granularity=360, target_distance=0.01, episode_length=400,
                 link_mode=LinkMode.FIXED,
                 angle_mode=AngleMode.INSTANT,
                 reward_mode=RewardMode.INVERSE,
                 env_type=EnvType.REACHER_3_DOF,
                 link_lengths=FIXED_LINK_LENGTHS, random_target=True):
        ...
        self.fixed_lengths = link_lengths
        ...
        self.observation_space = spaces.Box(low=0., high=1., shape=(18 if self.env_type == EnvType.REACHER_3_DOF else 13,),
    ...
    def step(self, action):
        ...
        if self.link_mode == LinkMode.ACTIONABLE or self.link_mode == LinkMode.OPTIMAL and self.link_lengths is None:
            if self.dof == 3:
                ...
                self.link_lengths = action[2:4] * MAX_LINK_LENGTH * multiplier
                self.link_lengths = np.append(self.link_lengths, 0)
            return self.__get_observation(), 0, False, {}
        ...
        if self.angle_mode == AngleMode.INSTANT:
            ...
            if self.dof == 3:
                ...
                self.joint_velocities = np.clip((action[0:2] - diff) * 2 * MAX_JOINT_VELOCITY * multiplier,
                                                -MAX_JOINT_VELOCITY, MAX_JOINT_VELOCITY)
                self.joint_velocities = np.append(self.joint_velocities, 0)
        ...
        if self.env_type == EnvType.REACHER_3_DOF:
            reward = -np.linalg.norm(end - self.target_position)
        else:
            if self.reward_mode == RewardMode.LINEAR:
                reward = - np.linalg.norm(j3 - self.target_2_position) - np.linalg.norm(j2 - self.target_position)
                         # - 10 * sum([abs(v) for v in self.joint_velocities])
            else:
                reward = 1/(np.linalg.norm(j3 - self.target_2_position) + np.linalg.norm(j2 - self.target_position))

        # if self.angle_mode == AngleMode.SIM:
        #     reward -= np.abs(self.joint_velocities).sum()
        done = self.n_steps >= self.episode_length
        self.ext_reward_history.append(reward)
        self.curiosity_loss_history.append(0)  # TODO
    ...
    def reset(self):
        ...
        if self.link_mode == LinkMode.RANDOM or self.link_mode == LinkMode.ACTIONABLE:
            if self.dof == 2:
                self.link_lengths = np.random.uniform(0, MAX_LINK_LENGTH, size=(2,))
                self.link_lengths = np.append(self.link_lengths, 0)
            else:
            ...
            if self.dof == 3:
                self.link_lengths = self.fixed_lengths
            else:
                self.link_lengths = np.array(self.fixed_lengths)
        elif self.link_mode == LinkMode.OPTIMAL:
            self.link_lengths = None

        if self.random_target:
            self.target_position = uniform_sphere(0.45)
            self.target_2_position = uniform_sphere(0.45)
        else:
